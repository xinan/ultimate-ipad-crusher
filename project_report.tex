...
\STATE $table \leftarrow 2DTable(N, L)$
\FOR{$i = 1$ \TO $N$}
	\STATE $table[i][1] = 1 + f(N-i)$
\ENDFOR
\FOR{$i = 1$ \TO $L$}
	\STATE $table[1][i] = i + f(N-1)$
\ENDFOR
\FOR{$i = 2$ \TO $N$}
	\FOR{$j = 2$ \TO $L$}
		\STATE $best \leftarrow \infty$
